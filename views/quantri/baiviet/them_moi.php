    ...
    <div class="form-container">
        ...
        <form method="POST" id="articleForm">
            ...
            <div class="form-group">
                ...
                <textarea id="content" name="content" class="summernote"><?php echo htmlspecialchars($article['content'] ?? ''); ?></textarea>
            ...
            <div class="form-group">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div>
                        <input type="file" id="thumbnail_file" name="thumbnail_file" accept="image/*" style="width: 100%;">
                        <div class="help-text" style="margin-top: 8px;">Chọn ảnh từ máy tính (JPG, PNG, GIF, WebP)</div>
                        <button type="button" id="upload_thumbnail_btn" class="btn btn-submit" style="margin-top: 10px; width: 100%;">↑ Tải lên</button>
                    </div>
                    <div>
                        <div id="thumbnail_preview" style="width: 100%; height: 200px; border: 2px dashed #ddd; border-radius: 5px; display: flex; align-items: center; justify-content: center; background-color: #fafafa; overflow: hidden;">
                            <?php if (isset($article['thumbnail']) && $article['thumbnail']): ?>
                                <img src="<?php echo htmlspecialchars($article['thumbnail']); ?>" alt="Preview" style="max-width: 100%; max-height: 100%; object-fit: cover;">
                            <?php else: ?>
                ...
                <input type="hidden" id="thumbnail" name="thumbnail" value="<?php echo htmlspecialchars($article['thumbnail'] ?? ''); ?>">
            </div>
                        ...
                        <?php foreach ($tags as $tag): ?>
                            <div class="tag-item">
                                <input type="checkbox" id="tag_<?php echo $tag['id']; ?>" 
                                       name="tag_ids[]" value="<?php echo $tag['id']; ?>"
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="status" name="status" required>
                        <option value="draft" <?php echo (isset($article['status']) && $article['status'] === 'draft') ? 'selected' : ''; ?>>
                            Nháp (Chỉ bạn thấy)
                        </option>
                        <option value="pending" <?php echo (isset($article['status']) && $article['status'] === 'pending') ? 'selected' : ''; ?>>
                            Chờ duyệt (Gửi cho Admin)
                        </option>
                        <option value="published" <?php echo (isset($article['status']) && $article['status'] === 'published') ? 'selected' : ''; ?>>
                            Đã đăng (Công khai)
                        </option>
                    </select>
                </div>
...
$(document).ready(function() {
            ...
            success: function(response) {
                try {
                    ...
                    if (response_obj.success && response_obj.url) {
                        ...
                        $('#thumbnail_preview').html('<img src="' + response_obj.url + '" style="max-width: 100%; max-height: 100%; object-fit: cover;">');
                    } else {
                        alert('❌ Lỗi: ' + (response_obj.error || 'Không thể tải ảnh'));
                    }
                } catch (e) {
    ...
    $('#thumbnail_file').on('change', function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(event) {
                $('#thumbnail_preview').html('<img src="' + event.target.result + '" style="max-width: 100%; max-height: 100%; object-fit: cover;">');
            };
    ...
    $('#articleForm').on('submit', function(e) {
        ...
        if (thumbnailFile && !thumbnailUrl) {
            e.preventDefault();
            alert('Ảnh đại diện chưa được tải lên. Vui lòng đợi hoặc bấm "Tải lên" trước khi đăng bài.');
            return false;
        }
